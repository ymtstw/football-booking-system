...
Vercel の Cron は **本番 URL にしか飛びません**。ローカルでロック → 編成 → 前日メールまで試すには、次のどちらかです。

- **`npm run cron:local-day-before`** … 下記スクリプトで JOB01〜03 を **順に** `GET` する（開発サーバーが起動している必要あり）
...
| `RESEND_API_KEY` / `RESEND_FROM` | **実メールまで試すとき必須**。無いと JOB03 は送信をスキップし `pending` のままになり得る |
...
4. 開催日・締切・予約データを整える  
   - JOB01 は `status = open` かつ **`reservation_deadline_at <= 実行時刻`** の行だけ `locked` にする  
   - 前日メール JOB03 は **実行時の「東京の今日」の翌日** の `event_date` だけが対象（`send-day-before-final` の実装どおり）
...
- **日付をずらしたいだけ**のテストでは、JOB03 はサーバー側の **東京の「今日」** に依存するため、**カレンダー上の前日に実行する**か、データと締切をその前提に合わせる必要があります。
