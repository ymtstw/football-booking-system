...
2. **案内メール（Cron）** … **16:00 JST** 想定で **マッチング案内**（`matching_proposal`）。対象は **東京の今日+2 日**が `event_date` の開催のみ（開催前日の人には送らない）。予約サイトの案内は **16:00頃**送信開始の目安＋到着の前後注記（`reserve-public-mail-schedule.ts` と一致）
...
5. **JOB03 前日最終メール** … 開催 **翌日（東京暦）** が `event_date` の行へ、予約代表メールで **確定＋雨天を1通**（**16:30 JST** 想定 Cron。予約サイトの案内は **16:30頃**送信開始の目安＋到着の前後注記）
