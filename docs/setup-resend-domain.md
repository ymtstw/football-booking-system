...
ドメイン検証前の **テスト用** 運用では、Resend は多くの場合 **API キー所有者（Resend アカウントに紐づくメール）宛だけ** に送信を許可します。  
予約完了メールの **宛先はチーム代表の `contact_email`**（予約フォームの入力）なので、**運営がログインしている Gmail とは別**です。他ドメイン宛に送ると `verify a domain` のようなエラーで `notifications.status = failed` になります。  
ローカルで自分宛に届けたいときは、テスト予約の代表メールを Resend 登録メールと同じにするか、早めに下記のとおりドメインを検証してください。

**補足:** 予約完了メールが利用者宛で失敗した場合、`OPS_NOTIFY_EMAIL`（未設定時は `.env.example` の既定）宛に、**別メールで運営通知**を送ります（確認コードは含めません）。テスト用 Resend では利用者宛が拒否されても、運営宛は届くことが多いです。
