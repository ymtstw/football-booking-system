...
- **締切後の自動編成**: 締切後の `matching/run`（または締切 Cron 内の同一処理）で **可能な限り枠に試合行を付与**する（午前は空枠解消を特に重視。詳細は 2b/2c）。締切の運用既定は **開催 2 日前 15:00（JST）** で、本番は **`/api/cron/lock-event-days` が `locked` 直後に編成まで実行**する（`vercel.json` 正本）。手動・リカバリ用に `GET /api/cron/run-matching-locked` も残す（**`locked` 滞留の再試行**。締切 Cron 本体は `open` のみ対象のため、運用の詳細は **behavior-catalog §1.6**）。  
