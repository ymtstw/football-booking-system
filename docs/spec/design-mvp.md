...
| 項目      | 内容                                                                                |
| ------- | --------------------------------------------------------------------------------- |
| 版       | MVP最終版 / 2026-04-06（枠数・代表学年・午後編成を 2026-04-15 追記。枠編集の予約ガード・強制 API を 2026-04-15 追記。同日予約上限を枠 capacity 合計に 2026-04-16 追記。本番/ステージング/メール送信のホスト分離を §1-5 に 2026-04-16 追記） |
| 作成      | 鹿島 大和                                                                             |
| 更新日     | 2026-04-16（同日予約上限を 3+3→6 / 4+4→8 に枠連動。§1-5 ホスト・送信ドメイン方針を追記）                                                             |
...
| 通知     | 最小構成のみ   | 予約完了 / **前日13:30の最終版メール**（開催確定＋対戦予定＋雨天判断を1通にまとめる）／**例外時のみ**雨天中止の即時メール。催促や再送自動化はMVP外とする。   |
...
| 予約締切  | 開催日前日12:00              | 昼食数変更締切も同時刻              |
| 取消締切  | 開催日前日12:00              | 締切後はWeb取消不可              |
...
雨天判断は**原則として前日に行い**、管理画面で go / cancel を**事前登録**できる。参加者向けの標準通知は **前日 13:30（JST）の「最終版」1通**に、開催確定／雨天中止の文面・直近の判断メモ・（確定済みなら）対戦予定をまとめて送る。**荒天などで早く伝える必要がある場合のみ**、管理者は中止登録と同時に **即時メール（雨天中止の緊急通知）** を送れる（通常運用は 13:30 一括）。cancel の場合、weather_status = cancel かつ event_day.status = cancelled_weather に更新する。go の場合、weather_status = go とし、event_day.status は中止から戻すときは confirmed に復帰させ、それ以外は locked / confirmed 等の直前状態を維持する。
...
| 5   | 締切。前日12:00で予約・取消・昼食変更を停止                             | event_days.status = locked       |
| 6   | 午前補完・午後編成を実行し、前日確定を作成（12:01 想定）                      | matching_runs, match_assignments |
| 7   | 雨天判断。go / cancel を登録（**通知は原則 13:30 の最終版に含める**／例外は即時） | weather_decisions                |
...
| POST /api/admin/event-days/{id}/weather-decision | 雨天判断登録       | admin        | go / cancel を保存。**標準**は前日13:30のJOB03に反映。**オプション**で中止の即時メール可                                                                             |
...
| JOB | タイミング（JST・前日） | 処理                         | 備考                                                        |
| --- | ------------- | -------------------------- | --------------------------------------------------------- |
| 01  | 12:00         | 予約締切ロック                    | reservation / meal update を停止。Cron: UTC `0 3 * * `*       |
| 02  | 12:01         | 午前補完・午後一括編成                | 3チーム以上で実行（上限は枠定員に連動）。結果確定。Cron: UTC `1 3 * * *`                      |
| 03  | 13:30         | **最終版メール送信**（前日確定＋雨天判断を1通） | 予約者へ。**開催確定**と**雨天中止**のいずれにも対応した本文。Cron: UTC `30 4 * * `* |
| 04  | （例外時のみ）       | 雨天中止の**即時**通知              | 管理者が登録時に明示した場合のみ。標準は JOB03 に統合                            |


**MVPで送る通知**

- 予約完了
- 前日 **13:30** の最終版（対戦予定＋雨天判断の統合。中止日は「中止の最終版」として同一テンプレート）
- 雨天中止の **緊急即時**（管理者オプション・例外的運用のみ）
...
5. matching_run を current にし、event_day.status を confirmed に更新する（**参加者向けの最終メールは原則 JOB03・前日 13:30** で送る）。
...
| 通知                 | 予約完了 / 前日13:30最終版（雨天含む）／緊急時のみ即時中止メール                                            | 確定                      |
