...
| 置き場所 | 入れる値 |
|----------|-----------|
| **Vercel → Production** | 本番 Supabase の Project URL / Publishable / Secret |
...
- **既にデータが入った Staging を「業務テーブルだけ空」にしたい場合:** SQL Editor で [supabase-truncate-all-public-data.sql](./ops/supabase-truncate-all-public-data.sql) を実行（**本番では実行しない**）。実行後は `app_admins` を再登録する。
