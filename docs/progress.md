...
- [x] 雨天判断を前日一括最終メール（16:30 頃バッチ・利用者向け目安 17:30 まで）に含める（例外: 即時 `weather_cancel_immediate`・`day_before_17` 雨天予約）
...
- **2026-04-16** — 運用スケジュール正への文書同期: 締切 **2日前15:00**・編成 **15:01**・案内 **16:30**・最終 **17:00** JST 目安。対象: `implemented-behavior-catalog.md`、`mvp-day-before-runbook.md`、`local-day-before-cron.md`、`vercel-production-checklist.md`、`progress.md`、`run-local-day-before-cron.mjs`（案内 Cron を連続実行に追加）、`implemented-matching-algorithm.md`。
- **2026-04-23** — Cron 前倒しと利用者向け目安の整理: マッチング案内 **`0 7`**（16:00 JST・目安 17:00 まで）、前日最終 **`30 7`**（16:30 JST・目安 17:30 まで）。案内と最終の **30 分 stagger**。編成の **定期 `1 6` は廃止**（締切 Cron 内が主）。`reserve-public-mail-schedule.ts`・予約画面・管理説明・`vercel.json`・主要ドキュメントを同期。
