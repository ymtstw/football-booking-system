...
- [x] 開催日作成時に既定枠（午前4・午後4）の `event_day_slots` を自動生成する ※`src/domains/event-days/default-slots.ts` と `POST /api/admin/event-days`
- [x] `slot_code`（MORNING_1〜4、AFTERNOON_1〜4 等）の一意性を担保する（※DB `UNIQUE(event_day_id, slot_code)` ＋初期データで固定コード）
...
- [x] 前日 13:00（タイムゾーン方針を決め、JST 等を文書化）で `locked` にする処理を実装する（`GET /api/cron/lock-event-days`・`open` かつ `reservation_deadline_at <= now()` を一括 `locked`。Cron は UTC 04:00 = 同日 JST 13:00。仕様: `docs/spec/reservation-deadline-and-event-status.md`）
...
- [ ] vercel.json 等で JOB01〜03 のスケジュールを登録する（タイムゾーンを確認）（**2026-04-13:** JOB01 締切ロックのみ登録済み。JOB02〜03 は未）
