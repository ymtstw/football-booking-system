...
**正本の仕様（リポジトリ内のファイル）:** `docs/spec/README.md`（読み順）・`docs/spec/implemented-behavior-catalog.md`（挙動）・`docs/spec/implemented-system-overview.md`（俯瞰）  
...
- [x] `reservations` テーブルを作成する（確認コードのハッシュ・`public_ref`・participant_count CHECK 等。詳細はマイグレーション正本）
...
- [x] `reservation_deadline_at`（**開催2日前15:00 JST** を既定にしつつ任意時刻）を保持し、API で参照する（※`POST` で ISO 8601 受け取り。既定算出は `src/lib/dates/reservation-deadline-default.ts`）
...
- [x] 確認コード（新規は短い英数字・旧は 64 hex 互換）と予約番号（`public_ref`）を生成し、確認コードの DB 保存はハッシュのみ、レスポンスでは確認コード平文と予約番号を返す（仕様: `docs/spec/implemented-behavior-catalog.md` §5.1）
...
### 1-7. 予約照会・取消 API（設計書 8-3。識別子の正本は `implemented-behavior-catalog.md` §5.1）

- [x] `GET /api/reservations/{token}` でハッシュ照合する（`[token]` は確認コード。予約番号のみでは照会不可）
...
- [x] SCR-02 予約完了（予約番号・確認コードの表示・コピー・入力確認）
- [x] SCR-03 予約照会・取消・締切前の内容変更（確認コード入力・保存完了の視覚的フィードバック含む）
...
- [x] **開催2日前15:00（JST）** を標準締切の既定とし、`reservation_deadline_at <= now()` で締切到達分を処理する（`GET /api/cron/lock-event-days`・Cron UTC **`0 6`** = 同日 JST **15:00**。active 3 未満は `cancelled_minimum`＋最少中止メール。仕様: `docs/spec/implemented-behavior-catalog.md` §1）
...
- [x] 1 チームのみの午前枠を優先して埋める（アルゴリズム詳細は `docs/spec/implemented-matching-algorithm.md`）
...
- [x] vercel.json で締切・案内・前日最終を登録する（UTC **`0 6`** 締切15:00 / **`0 7`** マッチング案内16:00 / **`30 7`** 前日最終16:30 JST 目安。編成の主経路は締切 Cron 内。`run-matching-locked` は手動用）
- [x] Cron 用エンドポイントをシークレットヘッダ等で保護する（`CRON_SECRET`・Bearer 不一致は 401）

### 2-10. 開催最低人数（設計書 2-2）

- [x] 締切時点で active が 3 未満のときの扱い（`cancelled_minimum` 等）を実装し、運用手順に書く（`process-reservation-deadline`・runbook）
...
- [x] 雨天判断を前日一括最終メール（16:30 頃バッチ・利用者向け表記 16:30頃＋到着注記）に含める（例外: 即時 `weather_cancel_immediate`・`day_before_17` 雨天予約）
...
- [ ] （将来）team 統合 UI と監査ログ
- [ ] （将来）lane による1面2分割
- [ ] （将来）利用者向けの確認コード再送 API（管理画面の予約完了メール再送とは別。旧設計書 8-3-1 メモ）
...
- [x] GET /api/reservations/{token}（`token` = 確認コード。予約番号不可）
...
- [x] JOB01 締切ロック（**15:00 JST** 想定トリガー）※`GET /api/cron/lock-event-days`・Vercel Cron **`0 6 * * *`**
- [x] 午前補完・午後一括編成（**締切直後**が主経路）※`POST /api/admin/matching/run`・`GET /api/cron/run-matching-locked` と同一ロジック。定期の **`1 6` Cron は廃止**（手動・ローカル用ルートは残す）
- [x] マッチング案内メール（**16:00 JST** 想定）※`GET /api/cron/send-matching-proposal`・**`0 7 * * *`**
- [x] JOB03 前日最終メール（**16:30 JST** 想定）※`GET /api/cron/send-day-before-final`（`day_before_final`）・**`30 7 * * *`**
- [x] JOB04 雨天判断の例外通知（登録時オプションで即時 `weather_cancel_immediate`）。標準文面は JOB03 に統合
...
- [x] 通知が予約完了・最少中止・案内・前日最終版（雨天含む）・緊急雨天をカバーする（テンプレ: `reservation_created` / `minimum_cancel_notice` / `matching_proposal` / `day_before_final` / `weather_cancel_immediate`）
...
正本仕様は `docs/spec/implemented-behavior-catalog.md`（API・Cron・状態）および `docs/spec/implemented-system-overview.md`。チェックは `docs/progress.md` の 1-3〜1-10 と末尾の API 一覧。
...
- **`reservation_deadline_at`** は「**開催2日前 15:00（JST）** を運用上の標準既定」とし、メモ欄の「タイムゾーン」に一文書く（例: DB は `timestamptz`、計算は Asia/Tokyo）。
...
1. **`POST /api/admin/event-days`**（または REST に沿ったパス）  
   - リクエスト例: `event_date`, `grade_band`, `status`（初期 `draft`）, `reservation_deadline_at`（またはサーバー側で **開催2日前15:00 JST** から算出）。  
...
10. **確認コード・予約番号:** 確認コードは十分なエントロピーのランダム（新規は短い英数字列。DB はハッシュのみ）。**予約番号**（`public_ref`）は表示・問い合わせ用で、照会パスの `[token]` には使わない。レスポンスは確認コード平文を **この応答で一度**返す。  
...
1. **`GET /api/reservations/[token]`**  
   `[token]` は **確認コード**（新形式または旧 64 hex）。正規化後にハッシュ化して `reservation_token_hash` と照合。開催日から **30 日超**は 404 等（設計 §8-3）。**`public_ref` のみでは照会できない**こと（§5.1）。
...
4. **レート制限**  
   Middleware または Route 内で IP / 確認コード照会単位の粗い制限（実装済みの失敗記録・429 等。詳細はコード）。

5. **ログ**に確認コード平文・個人情報全文を残さない。
...
3. **SCR-02** 完了画面で予約番号・確認コードを表示。  
4. **SCR-03** 確認コード入力で照会・取消・締切前の PATCH 変更。保存完了は画面上で明示。  
5. **お問い合わせ** `/reserve/contact` はプレースホルダー（メール API 未接続）。  
6. 合宿導線は案内＋相談フォーム（受付〜事前案内まで。`docs/spec/mvp-product-intent.md` 参照）。

---

#### I. 通知（0-1・1-6）

Resend 等のアカウントと API キー（0-1）が決まったら、予約完了時にメール送信。失敗時は `notifications` に `failed` で残す（`docs/spec/implemented-behavior-catalog.md` §3・progress 1-6）。
...
- **2026-04-14** — `GET /api/admin/matches`、`POST /api/admin/matching/run` と DB RPC `admin_apply_matching_run`（`20260421100000`）。**各環境で `db push`。** 再実行は afternoon 済みなら 409。編成後 `confirmed`。**同日** 午前ペアのグローバル最適化・0枠埋め（3〜6 のみ）・午後2巡目（6 未満）・審判は次枠優先。実装仕様は `docs/spec/implemented-matching-algorithm.md`。
- **2026-04-15** — 枠管理: active 予約がある開催日は通常 `PATCH/POST .../slots` を 409 で拒否。`.../slots/force` ＋ `/admin/event-days/{id}/slots/force` で同意確認付き強制変更。共有処理 `admin-event-day-slot-mutations.ts`。`docs/spec/implemented-behavior-catalog.md`（枠・締切）を更新。進捗チェックリストを実装状況に同期（Phase 2 編成コア・枠 API・SCR-14 等）。
...
- **2026-04-16** — 運用スケジュール正への文書同期: 締切 **2日前15:00**・編成は締切 Cron 内・案内 **16:00頃**・最終 **16:30頃** JST（利用者向けは `reserve-public-mail-schedule.ts` と一致）。対象: `implemented-behavior-catalog.md`、`mvp-day-before-runbook.md`、`local-day-before-cron.md`、`vercel-production-checklist.md`、`progress.md`、`run-local-day-before-cron.mjs`（案内 Cron を連続実行に追加）、`implemented-matching-algorithm.md`。
- **2026-04-23** — Cron 前倒しと利用者向け表記の整理: マッチング案内 **`0 7`**（16:00 JST・表記 **16:00頃**）、前日最終 **`30 7`**（16:30 JST・表記 **16:30頃**）。案内と最終の **30 分 stagger**。編成の **定期 `1 6` は廃止**（締切 Cron 内が主）。`reserve-public-mail-schedule.ts`・予約画面・管理説明・`vercel.json`・主要ドキュメントを同期。
- **2026-04-27** — 予約の識別子仕様を文書化: **予約番号**（`public_ref`）と**確認コード**（新規は短い英数字・DB はハッシュ、旧 64 hex 互換）。`implemented-behavior-catalog.md` §5.1 新設・昼食 §5.2 へ繰り下げ。`implemented-system-overview.md`・`mvp-product-intent.md`・本 `progress.md` の関連記述を同期。
