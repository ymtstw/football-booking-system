...
- [x] vercel.json で締切・編成・案内・最終を登録する（UTC **`0 6`** 締切15:00 / **`1 6`** 編成15:01 / **`30 7`** 案内16:30 / **`0 8`** 最終17:00 JST 目安）
...
- [x] 雨天判断を前日17:00の最終メールに含める（例外: 即時 `weather_cancel_immediate`・`day_before_17` 雨天予約）
...
- [x] JOB02 午前補完・午後一括編成（**15:01 JST** 想定）※`GET /api/cron/run-matching-locked`・`POST /api/admin/matching/run` と同一ロジック・**`1 6 * * *`**
- [x] マッチング案内メール（**16:30 JST** 想定）※`GET /api/cron/send-matching-proposal`・**`30 7 * * *`**
- [x] JOB03 前日最終メール（**17:00 JST** 想定）※`GET /api/cron/send-day-before-final`（`day_before_final`）・**`0 8 * * *`**
