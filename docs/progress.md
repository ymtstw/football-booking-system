...
- [x] **開催2日前15:00（JST）** を標準締切の既定とし、`reservation_deadline_at <= now()` で締切到達分を処理する（`GET /api/cron/lock-event-days`・Cron UTC **`0 6`** = 同日 JST **15:00**。active 3 未満は `cancelled_minimum`＋最少中止メール。仕様: `docs/spec/reservation-deadline-and-event-status.md`）
...
- **2026-04-16** — 運用スケジュール正への文書同期: 締切 **2日前15:00**・編成 **15:01**・案内 **16:30**・最終 **17:00** JST 目安。対象: `design-mvp.md`、`reservation-deadline-and-event-status.md`、`mvp-day-before-runbook.md`、`local-day-before-cron.md`、`vercel-production-checklist.md`、`progress.md`、`run-local-day-before-cron.mjs`（案内 Cron を連続実行に追加）、`matching-algorithm-impl.md`。
