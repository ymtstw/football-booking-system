...
function baseCreateRpcParams(eventDayId: string, morningSlotId: string, tokenHash: string, emailSuffix: string) {
  return {
    ...
    p_contact_email: `apply-match-${emailSuffix}@example.test`,
...
describe.skipIf(!hasSupabaseEnv())("integration: admin_apply_matching_run", () => {
  it("locked 以外は not_locked で拒否（assignments 空でも DB は変えない）", async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.rpc("admin_apply_matching_run", {
        p_event_day_id: eventDayId,
        p_assignments: [],
      });
      expect(error).toBeNull();
      expect(data).toMatchObject({ success: false, error: "not_locked" });
  ...
  it("open→予約3件→locked→buildMatchingAssignments→RPC で confirmed かつ割当が付く", async () => {
    const { eventDayId } = await insertEventDayWithSlots({
      status: "open",
      reservationDeadlineAtIso: futureDeadlineIso,
    });
    try {
      ...
      const { error: lockErr } = await supabase.from("event_days").update({ status: "locked" }).eq("id", eventDayId);
      ...
      expect(dayAfter?.status).toBe("confirmed");
      ...
      const { count, error: cntErr } = await supabase
        ...
        .eq("matching_run_id", run!.id as string);
