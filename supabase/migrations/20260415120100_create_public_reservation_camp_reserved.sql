-- camp_status = reserved の開催日は日帰り予約（create_public_reservation）を拒否
...
BEGIN
  ...
  END IF;
  IF ed.camp_status = 'reserved'::public.event_day_camp_status THEN
    RETURN jsonb_build_object('success', false, 'error', 'camp_reserved');
  END IF;
...
COMMENT ON FUNCTION public.create_public_reservation IS '公開予約作成。合宿占有（camp_status=reserved）の日は拒否。service_role / Route Handler からのみ。';
