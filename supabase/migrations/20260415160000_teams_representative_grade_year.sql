-- チームの代表学年（1〜6）。予約時に開催日の学年帯に合わせて選択。
ALTER TABLE public.teams
  ADD COLUMN IF NOT EXISTS representative_grade_year smallint;

ALTER TABLE public.teams DROP CONSTRAINT IF EXISTS teams_representative_grade_year_chk;

ALTER TABLE public.teams
  ADD CONSTRAINT teams_representative_grade_year_chk CHECK (
    representative_grade_year IS NULL
    OR (
      representative_grade_year >= 1
      AND representative_grade_year <= 6
    )
  );

COMMENT ON COLUMN public.teams.representative_grade_year IS '参加児童の代表学年（1〜6）。複数学年が混ざる場合は多い側に合わせて登録。';

-- 公開予約 RPC: 代表学年を必須にし、開催日 grade_band と整合チェック
...
'公開予約作成。代表学年（1〜6）必須・開催日 grade_band と整合。';
