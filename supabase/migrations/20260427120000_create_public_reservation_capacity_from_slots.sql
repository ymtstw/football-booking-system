-- 同日の予約上限を「有効な午前枠の capacity 合計」にする（3+3 で 6、4+4 で 8）。
-- 枠内は event_day_slots.capacity に従う（従来は常に 2 相当）。
...
'公開予約作成。代表学年（1〜6）必須・開催日 grade_band と整合。同日の active 上限は有効な午前枠の capacity 合計。';
