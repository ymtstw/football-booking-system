-- Per-event-day lunch menu overrides (optional). Empty junction = use global active lunch_menu_items.
-- create_public_reservation: validate lunch line items against that effective set.

CREATE TABLE public.event_day_lunch_menu_items (
  event_day_id uuid NOT NULL REFERENCES public.event_days (id) ON DELETE CASCADE,
  lunch_menu_item_id uuid NOT NULL REFERENCES public.lunch_menu_items (id) ON DELETE CASCADE,
  sort_order integer NOT NULL DEFAULT 0,
  created_at timestamptz NOT NULL DEFAULT now(),
  PRIMARY KEY (event_day_id, lunch_menu_item_id)
);

CREATE INDEX event_day_lunch_menu_items_event_day_idx
  ON public.event_day_lunch_menu_items (event_day_id, sort_order, lunch_menu_item_id);

COMMENT ON TABLE public.event_day_lunch_menu_items IS
  '開催日ごとの昼食上書き。0件ならグローバル昼食マスタ（is_active）を使用。1件以上あるときはその中からのみ予約で選択可。';

ALTER TABLE public.event_day_lunch_menu_items ENABLE ROW LEVEL SECURITY;

CREATE POLICY event_day_lunch_menu_items_admin_all
  ON public.event_day_lunch_menu_items
  FOR ALL
  TO authenticated
  USING (public.is_app_admin())
  WITH CHECK (public.is_app_admin());

-- create_public_reservation: 昼食重複チェックで HAVING に SELECT 別名を使わない（mid_key エラー回避）。
...
'公開予約作成。昼食は p_lunch_items JSON 配列 [{menu_item_id, quantity}]（quantity 0 は無視）。開催日に event_day_lunch_menu_items が無いときはグローバル有効メニューのみ可。あるときはその組み合わせに含まれる有効メニューのみ可。税込単価は明細にスナップショット。';
