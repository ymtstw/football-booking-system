...
/** 対応状況（ステータス選択・保存） */
export function CampInquiryStatusSection({
  inquiryId,
  initialStatus,
}: StatusProps) {
  ...
  return (
    <section
      ...
      aria-labelledby="camp-inquiry-status-heading"
    >
      <h2
        id="camp-inquiry-status-heading"
        className="text-sm font-semibold text-zinc-900"
      >
        対応状況
      </h2>
      <div className="mt-3 flex flex-wrap items-center gap-2">
        <label htmlFor={`camp-inquiry-status-${inquiryId}`} className="sr-only">
          ステータス
        </label>
        <select
          id={`camp-inquiry-status-${inquiryId}`}
          value={status}
...
/** 連絡する（外部メール・コピー） */
export function CampInquiryContactSection({
  contactEmail,
  contactPhone,
  outlookWebHref,
  mailtoHref,
  ...
  return (
    <section
      className="rounded-lg border border-zinc-200 bg-white p-4 sm:p-5"
      aria-labelledby="camp-inquiry-contact-heading"
    >
      <h2
        id="camp-inquiry-contact-heading"
        className="text-sm font-semibold text-zinc-900"
      >
        連絡する
      </h2>
