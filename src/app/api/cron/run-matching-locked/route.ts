/**
 * `locked` な開催日に対し午前補完・午後一括編成を順に実行（旧 Cron JOB02 と同一ロジック）。
 *
 * **Vercel の定期実行からは外した**（締切は `/api/cron/lock-event-days` 内でロック直後に編成まで実行）。
 * 手動リカバリ・ローカル検証・運用スクリプトから `GET` / `POST` する用途で残す。
