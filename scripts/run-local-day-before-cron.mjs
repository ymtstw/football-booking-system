 ...
 * 1) lock-event-days  2) run-matching-locked  3) send-day-before-final
 *
...
if (checkOnly) {
  ...
  console.log("[check] メール送信には RESEND_API_KEY / RESEND_FROM も必要です（未設定なら JOB03 で送信スキップ）。");
  process.exit(0);
}
...
async function main() {
  console.log("ローカル前日 Cron 連続実行（JOB01 → JOB02 → JOB03）");
