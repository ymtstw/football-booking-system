...
export function registerExhaustiveRows(ctx) {
  ...
  const wxRows = [
    ...
    ["TC-EX-WX-422I3", "day_before_17+即時同時", "異常", "422 前日17:00予約と即時メールは同時に指定できません", "両方 true"],
    ...
    ["TC-EX-WX-409DB17A", "前日17予約+最終通知済", "異常", "409 最終通知が完了しているため、前日17:00の雨天中止予約はできません", "final_day_before_notice_completed_at あり"],
    ["TC-EX-WX-409DB17B", "前日17予約+状態不適合", "異常", "409 前日17:00の雨天中止予約は、確定または締切済の開催日のみ", "status=open のみ等"],
